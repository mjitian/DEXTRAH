...
python -m torch.distributed.run --nnodes=1 --nproc_per_node=1 \
  train.py \
    ...
    agent.params.config.mini_epochs=8 \
    agent.params.config.multi_gpu=True \
    agent.wandb_activate=False \
    env.success_for_adr=0.2 \
    env.objects_dir=visdex_objects \
    env.adr_custom_cfg_dict.fabric_damping.gain="[5.0, 10.0]" \
    env.adr_custom_cfg_dict.reward_weights.finger_curl_reg="[-0.005, -0.005]" \
    env.adr_custom_cfg_dict.reward_weights.lift_weight="[10.0, 0.0]" \
